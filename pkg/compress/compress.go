...
func (c *Compressor) Compress(snap *snapstore.Snapshot) (*snapstore.Snapshot, error) {
	if !c.enabled {
		return snap, nil
	}
	snap.SnapName = snap.SnapName + c.extension
	return snap, archiver.Archive([]string{snap.SnapDir}, path.Join(snap.SnapDir, snap.SnapName))
}
...
func (c *Compressor) Decompress(snap *snapstore.Snapshot) error {
	...
	return archiver.Unarchive(path.Join(snap.SnapDir, snap.SnapName), filepath.Dir(snap.SnapDir))
}
